...
function userToResult(
  ...
  return {
    ...
    iconURL: user
      ? user.avatar
        ? `${CDN_URL}/avatars/${user.avatar._id}`
        : `${API_URL}/users/${user._id}/default_avatar`
      : undefined,
...
function channelToResult(channel: API.Channel): SearchResult {
  return {
    ...
    iconURL: (channel as any).icon
      ? `${CDN_URL}/icons/${(channel as any).icon._id}`
      : "",
...
function serverToResult(server: API.Server): SearchResult {
  return {
    ...
    iconURL: (server as any).icon
      ? `${CDN_URL}/icons/${(server as any).icon._id}`
      : "",
      ...
      .then((reports) =>
        reports.forEach((report) =>
          results.push({
            ...
            link: `/panel/mod/legacy/reports/${report._id}`,
          }),
        ),
      );
