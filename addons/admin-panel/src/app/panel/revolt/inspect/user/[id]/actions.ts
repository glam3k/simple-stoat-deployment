...
import { ABUSE_EMAIL, APP_URL } from "@/lib/config";
  ...
  if (type !== "ban") {
    ...
    await sendMessage(channel._id, {
      content: [
        ...
        "",
        type === "suspension"
          ? `Further violations may result in a permanent ban depending on severity, please abide by the [Acceptable Usage Policy](${APP_URL}/aup).`
          : `Further violations will result in suspension or a permanent ban depending on severity, please abide by the [Acceptable Usage Policy](${APP_URL}/aup).`,
        ...(caseId
          ? ["", `Case ID for your reference: **${caseId.substring(18)}**`]
          : ""),
        `If you have further questions about this strike, please contact ${ABUSE_EMAIL}`,
      ].join("\n"),
    });
  }
